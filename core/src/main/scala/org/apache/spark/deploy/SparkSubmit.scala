...
object SparkSubmit extends CommandLineUtils with Logging {
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    val clusterManager: Int = args.master match {
      ...
      case _ =>
        printErrorAndExit("Master must either be yarn or start with spark, mesos, k8s, " +
          "nomad or local")
        -1
    }
    ...
    val options = List[OptionAssigner](
      ...
      OptionAssigner(args.executorMemory, STANDALONE | MESOS | YARN | KUBERNETES |
        NOMAD, ALL_DEPLOY_MODES, confKey = "spark.executor.memory"),
      OptionAssigner(args.totalExecutorCores, STANDALONE | MESOS | KUBERNETES |
        NOMAD, ALL_DEPLOY_MODES, confKey = "spark.cores.max"),
